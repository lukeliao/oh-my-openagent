...
export const RuntimeFallbackConfigSchema = z.object({
  ...
  /** Maximum reactive fallback attempts per session before provider_exhausted summarying (default: 3) */
  max_fallback_attempts: z.number().min(1).max(20).optional(),
  /** Base cooldown in seconds before retrying a failed model when provider pacing does not supply Retry-After guidance (default: 60) */
